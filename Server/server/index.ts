  ...
  try {
    ...
    await db.execute(sql`
      CREATE TABLE IF NOT EXISTS stream_blocks (
        ...
        UNIQUE (stream_id, blocked_user_id)
      )
    `);
    await db.execute(sql`CREATE INDEX IF NOT EXISTS idx_stream_blocks_stream ON stream_blocks (stream_id)`);

    log("live_streams, stream_gifts, stream_viewers, stream_blocks tables ensured", "db");
  } catch (err) {
