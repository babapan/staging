...
export function registerAgencyRoutes(app: Express): void {
  ...
  app.post("/api/agency/my/hosts", async (req: Request, res: Response) => {
    ...
    try {
      ...
      console.error("[agency/my/hosts POST]", e?.message ?? e);
  ...
  // ── Ensure unique constraint on agency_hosts (agency_id, username) ─────────
  db.execute(sql`
    DO $$
    BEGIN
      IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
        WHERE conname = 'agency_hosts_agency_id_username_unique'
      ) THEN
        DELETE FROM agency_hosts ah
        WHERE id NOT IN (
          SELECT MAX(id) FROM agency_hosts GROUP BY agency_id, username
        );
        ALTER TABLE agency_hosts
          ADD CONSTRAINT agency_hosts_agency_id_username_unique UNIQUE (agency_id, username);
      END IF;
    END$$
  `).catch(console.error);
