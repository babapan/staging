...
/**
 * Cleanup job: runs every hour.
 * Resets vip_level → 0 and period_topup → 0 for users whose VIP has expired.
 * Users must top-up again to regain VIP status (7-day grace period model).
 */
export async function runVipExpiryCleanup(): Promise<void> {
  try {
    ...
    if (count > 0) {
      log(`[vip-expiry] expired VIP for ${count} user(s)`, "shop");
    }
  } catch (err: any) {
    console.error("[vip-expiry] cleanup error:", err?.message ?? err);
  }
}

export function startVipExpiryCleanup(): void {
  // Run immediately on startup then every hour
  runVipExpiryCleanup();
  const INTERVAL_MS = 60 * 60 * 1000; // 1 hour
  setInterval(runVipExpiryCleanup, INTERVAL_MS);
  log("VIP expiry cleanup job started (interval: 1h)", "shop");
}
