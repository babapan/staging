...
export class DatabaseStorage implements IStorage {
  ...
  // ── VIP — grace-period model (7 hari) ────────────────────────────────────
  // Tiers (berdasarkan period_topup kumulatif dalam periode aktif):
  //   1≥150k, 2≥300k, 3≥1.2jt, 4≥2jt, 5≥5jt koin
  // Setiap top-up menambah period_topup dan mereset expiry ke NOW()+7 hari.
  // Cron job (tiap jam) mereset vip_level=0 & period_topup=0 jika vip_expires_at <= NOW().
  async updateVipLevel(username: string, topupAmount: number): Promise<void> {
    try {
      await db.execute(sql`
        UPDATE user_profiles p
        SET
          lifetime_topup  = COALESCE(p.lifetime_topup, 0) + ${topupAmount},
          period_topup    = COALESCE(p.period_topup, 0) + ${topupAmount},
          vip_expires_at  = NOW() + INTERVAL '7 days',
          vip_level = CASE
            WHEN COALESCE(p.period_topup, 0) + ${topupAmount} >= 5000000 THEN 5
            WHEN COALESCE(p.period_topup, 0) + ${topupAmount} >= 2000000 THEN 4
            WHEN COALESCE(p.period_topup, 0) + ${topupAmount} >= 1200000 THEN 3
            WHEN COALESCE(p.period_topup, 0) + ${topupAmount} >= 300000  THEN 2
            WHEN COALESCE(p.period_topup, 0) + ${topupAmount} >= 150000  THEN 1
            ELSE 0
          END,
          updated_at = NOW()
        FROM users u
        WHERE p.user_id = u.id AND u.username = ${username}
      `);
    } catch { /* non-fatal — VIP update failure should not block top-up */ }
  }
  ...
  async redeemVoucher(code: string, username: string): Promise<Voucher> {
    ...
    // Update VIP level berdasarkan lifetime top up
    await this.updateVipLevel(username, voucher.amount);
