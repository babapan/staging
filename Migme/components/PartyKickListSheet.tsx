...
import { fetchPartyKicked, unkickPartyUser, PartyMemberEntry } from '../services/partyMemberService';
...
export default function PartyKickListSheet({ visible, onClose, roomId, isOwner }: Props) {
  ...
  const load = useCallback(async (isRefresh = false) => {
    ...
    const data = await fetchPartyKicked(roomId);
    setList(data);
    setLoading(false);
    setRefreshing(false);
  }, [roomId]);

  const handleUnkick = async (entry: PartyMemberEntry) => {
    Alert.alert(
      'Cabut Kick',
      `Izinkan @${entry.username} masuk kembali ke ruangan ini?`,
      [
        { text: 'Batal', style: 'cancel' },
        {
          ...
          onPress: async () => {
            ...
            const ok = await unkickPartyUser(roomId, entry.user_id);
            setActionId(null);
            if (ok) setList(prev => prev.filter(u => u.user_id !== entry.user_id));
            else Alert.alert('Gagal', 'Tidak dapat mencabut kick');
          },
        },
      ],
    );
    ...
    <Modal visible={visible} transparent animationType="none" statusBarTranslucent onRequestClose={onClose}>
      ...
      <Animated.View style={[s.sheet, { paddingBottom: insets.bottom + 8, transform: [{ translateY: slideAnim }] }]}>
        ...
        <View style={s.header}>
          ...
          <Text style={s.headerTitle}>daftar kick</Text>
          <View style={{ width: 22 }} />
        </View>
        ...
        {loading ? (
          ...
          <FlatList
            ...
            ListEmptyComponent={
              <View style={s.center}>
                <MaterialCommunityIcons name="account-remove-outline" size={44} color="rgba(255,255,255,0.15)" />
                <Text style={s.emptyText}>Tidak ada anggota yang dikick</Text>
              </View>
            }
            ...
            renderItem={({ item }) => (
              <View style={s.row}>
                ...
                <View style={s.info}>
                  ...
                  {item.kicked_by_username ? (
                    <Text style={s.sub}>Dikick oleh @{item.kicked_by_username}{item.kicked_at ? ` · ${fmtDate(item.kicked_at)}` : ''}</Text>
                  ) : null}
                </View>
                {isOwner && (
                  <TouchableOpacity
                    style={s.actionBtn}
                    onPress={() => handleUnkick(item)}
                    activeOpacity={0.7}
                    disabled={actionId === item.user_id}
                  >
                    {actionId === item.user_id
                      ? <ActivityIndicator size="small" color="#10B981" />
                      : <Text style={s.actionText}>Cabut</Text>
                    }
                  </TouchableOpacity>
                )}
              </View>
